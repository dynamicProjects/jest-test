...
describe("Test todo methods",()=>{
    ...
    it(`Returns a todo with id ${2}`, async done =>{
        await request(app).get("/todo/2").expect(200).then((response)=>{
            expect(response.body.length).toBe("Get pizza for dinner");
        });
        done();
    })
})

describe("Test response from quering and external API",()=>{
    it('Should reterive a random Chuck Norris joke', async done =>{
    ...
    it('No 2 Chuck Norris will be the same', async done =>{
